...
public class RSAUtil {
     ...
     * 生成密钥对字符串
     */
    private static void generateKeyPairString(int keySize) throws Exception{
        /** RSA算法要求有一个可信任的随机数源 */
        ...
        RSAUtil.publicModulus = Encodes.bytes2Hex(prime.toByteArray());
        RSAUtil.publicExponent = Encodes.bytes2Hex(exponent.toByteArray());
        ...
        RSAUtil.privateModulus = Encodes.bytes2Hex(privateModulus.toByteArray());
        RSAUtil.privateExponent = Encodes.bytes2Hex(privateExponent.toByteArray());
    }
    ...
    public static RSAPublicKey getRSAPublicKey(String hexModulus, String hexPublicExponent) {
        ...
        try {
            modulus = Encodes.hex2Bytes(hexModulus);
            publicExponent = Encodes.hex2Bytes(hexPublicExponent);
        } catch (Exception ex) {
    ...
    public static RSAPrivateKey getRSAPrivateKey(String hexModulus, String hexPrivateExponent) {
        ...
        try {
            modulus = Encodes.hex2Bytes(hexModulus);
            privateExponent = Encodes.hex2Bytes(hexPrivateExponent);
        } catch (Exception ex) {
...
//			return new String(Encodes.bytes2Hex(en_data));
        } catch (Exception ex) {
            ex.printStackTrace();
        }
        return null;
...
//			byte[] en_data = Encodes.hex2Bytes(encrypttext);
