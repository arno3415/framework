...
import com.wgoa.manager.utils.ShiroFilterUtils;
import com.wgoa.utils.message.IStatusMessage;
import com.wgoa.utils.response.ResponseResult;
...
public class KickoutSessionFilter extends AccessControlFilter {
    ...
    public void setCacheManager(CacheManager cacheManager) {
        ...
        this.cache = cacheManager.getCache("wl_");
    }
...
//            System.out.println((!deque.contains(sessionId))  + "---:::" + session.getAttribute("wgkickout"));
            if (!deque.contains(sessionId)
                    && (session.getAttribute("wgkickout") == null
                    || !(Boolean) session.getAttribute("wgkickout"))) {
                // 将sessionId存入队列
            ...
            while (deque.size() > maxSession) {
                ...
                try {
                    ...
                    if (kickoutSession != null) {
                        // 设置会话的kickout属性表示踢出了
                        kickoutSession.setAttribute("wgkickout", true);
                    }
            ...
            if (session.getAttribute("wgkickout") != null
                    && (Boolean) session.getAttribute("wgkickout")) {
                // 会话被踢出了
                try {
                    // 退出登录
            ...
            logger.error(
                    "控制用户在线数量【admin-->KickoutSessionFilter.onAccessDenied】异常！",
                    e);
    ...
    public static void out(ServletResponse response, ResponseResult result){
        ...
        try {
            ...
            out.print(objectMapper.writeValueAsString(result));//输出
            logger.debug("用户在线数量限制【manager-->KickoutSessionFilter.out】响应json信息成功");
        } catch (Exception e) {
            logger.error("用户在线数量限制【manager-->KickoutSessionFilter.out】响应json信息出错", e);
        }finally{
            if(null != out){
                out.flush();
                out.close();
